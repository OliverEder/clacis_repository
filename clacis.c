...
int main(void)
{
    int training_files=6;                  // Number of files for training
    int clasifying_files=236;               // Number of files to clasify
...
/*Pasa los valores de la imagen a una matriz*/
void set_image(int number_rgb)
{
    int count=0;
    ...
    for(int row = 0; row < number_rgb; row++)
    {
...
/*Muestra el valor de los pesos sinapticos*/
void show_neurons(int number_neurons)
{
    for(int row = 0; row < number_neurons; row++)
	{
        printf("%d  ",row );
        for(int channel = 0; channel < 3; channel++)
        {
...
void write_image(int number_rgb, char results,int rows,int cols)
{
    ...
    for(int row = 0; row < number_rgb; row++)
    {
        int neuron=*(image[row]+3);
        for(int channel = 0; channel < 3; channel++)
        {
            int newval=*(*(neurons+neuron)+channel)*255;
            //printf("%d  ",newval);
...
void trainig_image(int number_rgb, int number_neurons)
{
    ...
    for(int row = 0; row < number_rgb; row++)
    {
        ...
        calculate_distances(  number_neurons, 5, selected_pattern, &winner_neuron, &min_distance);
    
        //calculate new weights
        calculate_new_weights( selected_pattern, winner_neuron, min_distance, learning_rate);
...
void clasifying_image(int number_rgb, int number_neurons)
{
    ...
    for(int row = 0; row < number_rgb; row++)
    {
        ...
        //calculate distances and get the closest neuron
        classifying( number_neurons, 5, selected_pattern, &winner_neuron, &min_distance);                
    }
}
